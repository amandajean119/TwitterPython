...
class FollowClique:
    ...
    def __init__(self, tokens_ar, cluster):
        ...
        df = pd.read_csv('subset_edges.csv', names=['source', 'target'])

        self.cluster_name = cluster['cluster']
        print "Exploring based on seed cluster " + self.cluster_name
        # For the first round our clusters are lists of screen_names, not user_ids. Want to transform them to user_ids
        # since that is what the followers will be identified by in future rounds.
        users = cluster['users'].strip().split(';')
        self.n = float(len(users))
        self.total_possible_edges = 2.0 * (self.n-1.0)
        self.level = 0
        for user in users:
            # Can adjust this threshold based on clique, NEED TO EXPLORE
            user_edge_ratio = float(len(df[df['source'] == user]) + len(df[df['target'] == user]))/self.total_possible_edges
            if user_edge_ratio < .6:
                continue
            self.cur.execute('SELECT user_id FROM name_id WHERE screen_name = %s;', (str(user),))
            f = self.cur.fetchone()[0]
            if f:
                self.clique.add((f, self.level))
                self.to_check.add(f)
            else:
    ...
    def find_connected_followers(self):
        ...
        # WRITE THESE TO FILE OR DB IN CASE CODE FAILS??


def get_starting_cluster():
    df = pd.read_csv('subset_user_date_ratio.csv')
    # Can adjust this threshold, NEED TO EXPLORE
    clusters = df[df['ratio'] > .8]
    ...
    # need to add step to only include nodes with maybe > 0.6 ratio connectivity
    for index, cluster in clusters.iterrows():
        fc = FollowClique(tokens_ar, cluster)
        print "Initial size of cluster: " + str(fc.n)
        while fc.to_check:
            fc.explore()
            fc.find_connected_followers()
        clique = sorted(list(fc.clique), key=lambda c: c[1])
        with open('clique_expansion_' + fc.cluster_name + '.csv', 'w') as f:
